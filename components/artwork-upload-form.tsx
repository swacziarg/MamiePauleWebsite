...
export function ArtworkUploadForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!file || !description.trim()) {
      setMessage("Ajoutez une image et une description.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto flex w-full max-w-3xl flex-col gap-6 rounded-[2rem] border border-line bg-white/90 p-6 shadow-soft md:p-8">
      <h1 className="text-center text-3xl text-ink">Ajouter une œuvre</h1>
      ...
      >
        {previewUrl ? (
          ...
          <div className="space-y-3">
            ...
            <p className="text-base text-black/60">Format conseillé : JPG ou PNG</p>
          </div>
        )}
        <input
          ref={fileInputRef}
      ...
      <label className="space-y-2">
        <span className="text-lg text-ink">Description</span>
        <input
          required
          type="text"
          value={description}
          ...
          placeholder="Huile sur toile, paysage du matin..."
        />
      </label>
